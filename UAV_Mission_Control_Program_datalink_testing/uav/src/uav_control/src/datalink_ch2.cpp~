...
/**
 * Service callback sending datalink msgs
 */
...
int main(int argc, char **argv)
{
   ...
   */
  ros::init(argc, argv, "datalink_ch2");
  ...
  ch2_stat.msg_received = 0;
  ch2_stat.msg_sent = 0;
  ch2_stat.msg_crc_error = 0;

  /**
   * Create a service for sending data over datalink, on specified channel
   */
  ros::ServiceServer service = n.advertiseService("datalink_send_ch2", send);
  
  /**
   * Create a ROS publisher that publishes datalink frames received
   */
  datalink_publisher = n.advertise<uav_control::DFrame>("uav_control/datalink_in_ch2", 100);

  /**
   * Publishes statistics of msgs over this channel
   */
  stat_publisher = n.advertise<uav_control::channel_stat>("uav_control/datalink_stat_ch2", 15);
			...
			*/
			...
			if(CRCCnt == 4){
				...
				if(CRC_received == crcval){
					...
					//ROS_INFO("accepting message, publishing");
